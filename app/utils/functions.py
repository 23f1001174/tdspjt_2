...
Memory (System):  7.79GB (0.70GB free)
VM:               0%
Screen Reader:    no
Process Argv:     --crash-reporter-id 15b4fe18-9230-40d1-a841-bbd36833d230
...
CPU %   Mem MB     PID  Process
    0      157   32964  code main
    0       86    3588  ptyHost
    0        7   32572       conpty-agent
    0       81   35708       C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe -noexit -command "try { . \"c:\Users\2020v\AppData\Local\Programs\Microsoft VS Code\resources\app\out\vs\workbench\contrib\terminal\common\scripts\shellIntegration.ps1\" } catch {}"
    0        6   29376         C:\WINDOWS\system32\cmd.exe /c ""C:\Users\2020v\AppData\Local\Programs\Microsoft VS Code\bin\code.cmd" -s"
    0      105   36948           electron-nodejs (cli.js )
    1      131   23104             "C:\Users\2020v\AppData\Local\Programs\Microsoft VS Code\Code.exe" -s
    0       84   15536               crashpad-handler
    1       98   29952               gpu-process
    0       90   38672               utility-network-service
    0       70   12820  fileWatcher [1]
    0      307   17532  window [1] (encrypt.c - Visual Studio Code)
    4      134   25244  shared-process
    0      257   33584  extensionHost [1]
    0       54   31372       electron-nodejs (serverMain.js )
    0       29   34396       c:\Users\2020v\.vscode\extensions\ms-vscode.cpptools-1.23.3-win32-x64\bin\cpptools.exe
    0        6   22540         C:\WINDOWS\system32\conhost.exe 0x4
    0        2   28864         "c:\Users\2020v\.vscode\extensions\ms-vscode.cpptools-1.23.3-win32-x64\bin\cpptools.exe"
    0       24   31084           c:\Users\2020v\.vscode\extensions\ms-vscode.cpptools-1.23.3-win32-x64/bin/cpptools-srv.exe 34396 {A6E245FA-56CD-4241-B87A-33B635557BEF}
    0        7   26352             C:\WINDOWS\system32\conhost.exe 0x4
    0       18   35232     crashpad-handler
    0      136   36492     gpu-process
    3       39   37632     utility-network-service
""",
...
async def get_delhi_bounding_box() -> str:
    """
    Get the minimum latitude of Buenos Aires, Argentina using the Nominatim API

    Returns:
        Information about Buenos Aires's bounding box
    """
    try:
        import httpx
        import json
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not results:
                return "No results found for Buenos Aires, Argentina"

            # Find the correct Buenos Aires (capital city)
            delhi = None
            for result in results:
                if "Buenos Aires" in result.get("display_name", ""):
                    delhi = result
                    break

            # If we didn't find Buenos Aires specifically, use the first result
...
async def find_newest_seattle_github_user() -> str:
    ...
    try:
        ...
        # Parameters for the request
        params = {
            "q": "location:Zurich followers:>70",
            "sort": "joined",
            "order": "desc",
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not search_results.get("items"):
                return "No GitHub users found in Seattle with over 70 followers"

            # Get the newest user
            newest_user = None
            cutoff_date = datetime.fromisoformat(
                "2022-05-02T10:35:22Z".replace("Z", "+00:00")
            )
            ...
            # Create a detailed response
            return f"""
# Newest GitHub User in Zurich with 70+ Followers
...
"""
        ...
        return f"Error finding newest Zurich GitHub user: {str(e)}"
